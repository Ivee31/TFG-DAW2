...
export default function Dashboard() {
    ...
    const cargarMarcas = () => {
        setCargando(true);
        fetch('/api/index.php/marcas', { credentials: 'include' })
            .then(res => res.json())
            .then(data => {
    ...
    const handleEliminarMarca = (id_marca) => {
        setEliminando(true);
        fetch(`/api/index.php/marcas/${id_marca}`, { method: 'DELETE', credentials: 'include' })
            .then(res => res.json())
            .then(data => {
                setEliminando(false);
                ...
                <div className="bg-gris/40 backdrop-blur-sm p-6 md:p-8 rounded-2xl border border-white/5 shadow-2xl h-full flex flex-col">
                    ...
                    <div className="space-y-4 grow">
                        ...
                        {!cargando && marcasVisibles.map((m) => (
                            <div key={m.id_marca} className="relative group bg-oscuro/50 p-4 md:p-5 rounded-xl border border-transparent hover:border-ianuarius/50 transition duration-500">
                                ...
                                {confirmandoId === m.id_marca && (
                                    <div className="absolute bottom-full mb-2 right-0 z-20 bg-gris border border-ianuarius/50 rounded-xl p-4 shadow-2xl w-64">
